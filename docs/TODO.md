...
- [x] 从任意文件系统路径加载 PMX。
- [x] 从 ZIP 模型包加载 PMX，支持单模型自动导入和多模型选择。
...
- [x] 实现打开 PMX/ZIP 模型包和选择外部 WGSL 的原生文件对话框。
- [ ] 支持拖放 PMX、WGSL 和纹理文件。
...
- [x] 确定 Charme 项目文件扩展名为 `.charme`；目录结构待定。
